...
manifest. Sending goes out as one separate, scoped inquiry per supplier — no supplier
sees the rest of your list, and Manifest never handles payment.
...
- **Supplier** — a supplier's product list, with a running manifest summary.
- **Manifest** — event details, line items grouped by supplier (with per-item
